...
namespace FTN.ESI.SIMES.CIM.CIMAdapter
{
	public class CIMAdapter
	{
		...
		{
			Delta nmsDelta = null;
			ConcreteModel concreteModel = null;//za instance cimovskih klasa koje sam parsirala
			Assembly assembly = null;
			string loadLog = string.Empty;
			string transformLog = string.Empty;

			if (LoadModelFromExtractFile(extract, extractType, ref concreteModel, ref assembly, out loadLog))//ucitavanje konkretnog modela, konkretni model mapunjen instancama cimovskih klasa
			{
				DoTransformAndLoad(assembly, concreteModel, extractType, out nmsDelta, out transformLog);//sve podatke iz concreteModel-a prebaci u nmsDelta
			}
		...
		private bool LoadModelFromExtractFile(Stream extract, SupportedProfiles extractType, ref ConcreteModel concreteModelResult, ref Assembly assembly, out string log)
		{
			...
			return valid;
                                                                                      ...
                                                                                      //profil koji treba da obradjujem
					...
					case SupportedProfiles.Projekat:
						{
							...
							TransformAndLoadReport report = PowerTransformerImporter.Instance.CreateNMSDelta(concreteModel);//PTI ima svoju singletone reprezentaciju Instance koja ima metodu CreateNMDDelta koja primi cimovske klase/podatke, unutar sebe kreira deltu i vrati neki report

							if (report.Success)//ako je proslo uspijesno
							{
								nmsDelta = PowerTransformerImporter.Instance.NMSDelta;//onda ova klasa vrati deltu koju je napravila unutar sebe
								success = true;
							}
							else
							{
