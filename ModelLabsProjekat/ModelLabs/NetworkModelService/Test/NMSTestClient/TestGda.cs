...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
		...
		public List<long> GetExtentValues(ModelCode modelCode)
		{
			...
			XmlTextWriter xmlWriter = null;
			int iteratorId = 0; //BROJ rd koje metoda vrati, koliko RD je server nasao kod sebe, on te RD ne upisuje u listu kada ih nadje, vec im dodijeli neki broj (iteratorID) 
                //na osnovu kog zna koliko je klijent procitao RD-a 
			List<long> ids = new List<long>();//u ovoj listi cuvam GID-ove od RD koje nadjem 

			try
			{				
				int numberOfResources = 2;// po dva resursa odjednom povlacim
				int resourcesLeft = 0;

                List<ModelCode> properties = modelResourcesDesc.GetAllPropertyIds(modelCode);//GetAllPropertyIds() -> ova metoda primi model kod klase a kao rezultat vrati listu svih model kodova atributa te klase

                iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties);
				resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
								

				xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetExtentValues_Results.xml", Encoding.Unicode); //napravim fajl u kom ce se vrsiti upis procitanih RD
				xmlWriter.Formatting = Formatting.Indented;

				while (resourcesLeft > 0)
				{
					List<ResourceDescription> rds = GdaQueryProxy.IteratorNext(numberOfResources, iteratorId); //klijent govori serveru koliko RD hoce da procita
		...
		public List<long> GetRelatedValues(long sourceGlobalId, Association association) //VRACA MI ONE OBJEKTE KOJI REFERENCIRAJU KLASU zadatu preko sourceGlobalId
        {
			...
			{						
                ...
                //asocijacija u sebi sadrzi model kod atributa koji predstavlja listu referenci
                //sourceGlobalID - Prosljedjujem GID objekta za koji hocu da ispisem sve objekte koji ga referenciraju
                //properties - od onih objekata koji referenciraju (npr. ispisujem propertije od ACLINESERMENTA koji referenciraju PERLENSEQIMP)
        ...
        public void GetRFromSeriesCompensator()
        {
            string message = "Getting extent values method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            int numberOfResources = 2;
            int resourcesLeft = 0;
            ModelCode modelCode = ModelCode.SERIESCOMPENS;
            ...
            xmlTextWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\r_Results.xml", Encoding.Unicode);
            ...
            while (resourcesLeft > 0)
            {
                ...
                for (int i = 0; i < rds.Count; i++)
                {
                    float vrijednost = rds[i].GetProperty(ModelCode.SERIESCOMPENS_R).PropertyValue.FloatValue;
                    if(vrijednost > 5)
                    {
                        ids.Add(rds[i].Id);
                        rds[i].ExportToXml(xmlTextWriter);
                        xmlTextWriter.Flush();
                    }
        ...
        private Dictionary<DMSType, ResourceDescription> CreateResourcesToInsert()
        {
            ...
            SetACLineSegmentReferences(updates);
        ...
        private void SetTerminalReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            ...
            {
                if (updates[DMSType.TERMINAL].Properties[i].Id == ModelCode.TERMINAL_CONNECTNODE)
                {
                    updates[DMSType.TERMINAL].Properties[i].SetValue(updates[DMSType.CONNECTNODE].Id);
                }

                if (updates[DMSType.TERMINAL].Properties[i].Id == ModelCode.TERMINAL_CONDEQUIPM)
                {
                    updates[DMSType.TERMINAL].Properties[i].SetValue(updates[DMSType.SERIESCOMPENS].Id);
                }
            }
        }

        private void SetACLineSegmentReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.ACLINESEGM].Properties.Count; i++)
            {
                if (updates[DMSType.ACLINESEGM].Properties[i].Id == ModelCode.ACLINESEGM_PERLENIMP)
                {
                    updates[DMSType.ACLINESEGM].Properties[i].SetValue(updates[DMSType.PERLENSEQIMPEDANCE].Id);
                }
