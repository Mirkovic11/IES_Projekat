...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            tgda.GetExtentValues(ModelCode.PERLENSEQIMPEDANCE);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("Get PerLengthSequenceImpendances failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "6")
                    {
                        try
                        {
                            Association asocijacija = new Association();
                            List<long> lista = tgda.GetExtentValues(ModelCode.PERLENSEQIMPEDANCE);
                            long najveci = lista.Max();
                            asocijacija.Type = 0;
                            asocijacija.PropertyId = ModelCode.PERLENIMPENDANCE_ACLINESEGMS;
                            tgda.GetRelatedValues(najveci, asocijacija);
                            Console.WriteLine("\t*******Uspijesno pronadjeni svi ACLineSegments od perlengseqimp\nPERLENSEQIMPEDANCE sa najvecim ID-em je:" + najveci);
                    ...
                    else if (str == "7")
                    {
                        try
                        {
                           tgda.GetRFromSeriesCompensator();
                          /*  
                           *  ILI ovako
                           *  
                           *  
                           *  List<long> lista = tgda.GetExtentValues(ModelCode.SERIESCOMPENS);
                            ...
                            {
                                 rd.Add(tgda.GetValues(lista[i]));
                            }

                            XmlTextWriter xmlTextWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\r_Results.xml", Encoding.Unicode);
                            xmlTextWriter.Formatting = Formatting.Indented;
                            foreach (var r in rd)
                            {
                                if(r.GetProperty(ModelCode.SERIESCOMPENS_R).PropertyValue.FloatValue> 50)
                                {
                                    r.ExportToXml(xmlTextWriter);
                                    xmlTextWriter.Flush();
                                }
                            }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get PerLengthSequenceImpendances");
            Console.WriteLine("\t6) Get ACLineSegments associeted to PERLENSEQIMPEDANCE with highest GID");
            Console.WriteLine("\t7) Get SeriesCompensators where R > 10");
